import { okx, type Order as CcxtOrder } from 'ccxt';
...
export class OkxProvider extends Provider {
  readonly name = 'okx';

  private exchange: okx | null = null;
  // symbol cache for cancelOrder (OKX requires symbol to cancel)
  ...
  private ex(): okx {
    if (!this.exchange) throw new Error('OkxProvider: call connect() first');
